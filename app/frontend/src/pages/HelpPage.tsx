...
const USER_GUIDE = [
  ...
  '- 「AI 诊断」使用配置中心指定的 LLM 模型输出根因 / 建议 / 命令；置信度低于阈值（默认 0.75）会标记低置信。',
  ...
  '- 去重合并：扫描同错误类型 + 同服务且模板相似度 ≥80% 的案例簇（低于 80% 不展示合并建议），指定主案例后其余归档，合并走审批；已提交审批的案例组在提案处理期间不再重复统计，拒绝后恢复。',
  '',
  '## 5. 审批中心',
  '',
  '- 三个视图：待我审批 / 我发起的 / 我已处理。',
  ...
  '## 8. 知识健康风险闭环',
  '',
  '- 入口：「审计与配置 → 知识健康报表」，红 / 黄案例给出命中原因与逐案处置入口，处置操作需知识库管理员（kb_admin）及以上角色。',
  '- 同步死信（红）：点「重放死信任务」，系统重置死信任务并立即执行一次到期重试；成功后同步回到已确认，仍失败会按指数退避留在补偿队列。',
  '- 同步未确认（黄）：点单行「重试同步」触发到期补偿任务，或等待调度自动重试，无需人工干预。',
  '- 反馈分 ≤ -2 或超老化阈值且负反馈（红 / 黄）：点单行「归档」下线单个案例，或点「老化巡检（预览后归档）」先预览候选再批量归档；归档会同步删除检索侧索引并写审计。',
  '- 高风险内容（红，含误报放行留痕）：到「知识库」编辑案例内容重新提交（重新过内容安全扫描），或直接归档下线。',
  '- 已归档为生命周期终态，无需处理；处置完成后健康报表与运营总览自动刷新。',
].join('\n');
