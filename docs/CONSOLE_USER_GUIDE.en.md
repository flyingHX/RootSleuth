...
| diagnose_time_budget_seconds | Diagnosis wall-clock budget (30~600 seconds, default 90): the total duration cap for multi-round reasoning + forced conclusion; on timeout the system automatically degrades / returns 502, preventing unbounded request growth when the LLM slows down |
